    ...
    private PlayState attackPredicate(AnimationState state) {//判断是否处于攻击状态
        if(this.swinging && state.getController().getAnimationState().equals(AnimationController.State.STOPPED)) {
            ...
            switch (attackSkill){//随机关联攻击动画
                case 0:
                    state.getController().setAnimation(RawAnimation.begin().then("animation.treeguardian - Converted.attack1", Animation.LoopType.PLAY_ONCE));//攻击动作不循环
                    setAttackDamage(8.0f);//设置伤害
                    break;
                case 1:
                    state.getController().setAnimation(RawAnimation.begin().then("animation.treeguardian - Converted.attack2", Animation.LoopType.PLAY_ONCE));
                    setAttackDamage(10.0f);
                    break;
                case 2:
                    state.getController().setAnimation(RawAnimation.begin().then("animation.treeguardian - Converted.attack3", Animation.LoopType.PLAY_ONCE));
