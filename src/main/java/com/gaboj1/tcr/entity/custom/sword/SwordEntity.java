...
//                DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> ()-> rider =  Minecraft.getInstance().player);
        ...
        rider.getCapability(TCRCapabilityProvider.TCR_PLAYER).ifPresent(tcrPlayer -> {
            if(!tcrPlayer.isFlying()){
                tcrPlayer.setHasSwordEntity(false);
                TheCasketOfReveriesMod.LOGGER.info("sword entity "+ getId() + " rider is not flying. "+level());
                discard();
            }
        });
