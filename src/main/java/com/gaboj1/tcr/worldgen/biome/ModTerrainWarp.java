...
//import com.gaboj1.tcr.worldgen.dimension.noise.TFBlendedNoise;
...
public class ModTerrainWarp {
    ...
    public ModTerrainWarp(int width, int height, int yCount, BiomeSource source, NoiseSlider topslide, NoiseSlider bottomslide, NoiseSettings settings, BlendedNoise blend, NoiseModifier modifier) {
        ...
        this.dimensionDensityFactor = source instanceof ModBiomeProvider tfsource ? tfsource.getBaseFactor() : 1.0F;
        this.dimensionDensityOffset = source instanceof ModBiomeProvider tfsource ? tfsource.getBaseOffset() : 0.0F;
        this.caveNoiseModifier = modifier;
    }

    public void fillNoiseColumn(double[] adouble, int x, int z, int min, int max) {
        if (biomeSource instanceof ModBiomeProvider source) {
