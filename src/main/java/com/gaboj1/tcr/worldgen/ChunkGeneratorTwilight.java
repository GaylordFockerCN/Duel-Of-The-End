...
import com.gaboj1.tcr.util.Vec2i;
import com.gaboj1.tcr.worldgen.biome.*;
import com.gaboj1.tcr.worldgen.tf.ChunkGeneratorWrapper;
import com.gaboj1.tcr.worldgen.tf.ModBiomeProvider;
import com.gaboj1.tcr.worldgen.tf.ModTerrainWarp;
import com.gaboj1.tcr.worldgen.tf.TFNoiseInterpolator;
import com.gaboj1.tcr.worldgen.tf.noise.NoiseModifier;
import com.gaboj1.tcr.worldgen.tf.noise.NoiseSlider;
...
//    private final Map<ResourceKey<Biome>, ImmutableSet<TFLandmark>> biomeLandmarkOverrides;
    ...
    private final Optional<ModTerrainWarp> warper;

    private static final BlockState[] EMPTY_COLUMN = new BlockState[0];

    public ChunkGeneratorTwilight(ChunkGenerator delegate, Holder<NoiseGeneratorSettings> noiseGenSettings, boolean genDarkForestCanopy, Optional<Integer> darkForestCanopyHeight) {
        super(delegate);
...
//        this.biomeLandmarkOverrides = biomeLandmarkOverrides;
        ...
        if (noiseGenSettings.isBound()) {
            ...
            if (delegate.getBiomeSource() instanceof ModBiomeProvider source) {
                WorldgenRandom random = new WorldgenRandom(new LegacyRandomSource(0L));
                BlendedNoise blendedNoise = new BlendedNoise(random, 0.9999999814507745, 0.9999999814507745, 80.0, 160.0, 0.0D);
                NoiseModifier modifier = NoiseModifier.PASS;
                this.warper = Optional.of(new ModTerrainWarp(settings.getCellWidth(), settings.getCellHeight(), settings.height() / settings.getCellHeight(), source, new NoiseSlider(-10.0D, 3, 0), new NoiseSlider(15.0D, 3, 0), settings, blendedNoise, modifier));
            } else {
...
//        }
    ...
    private static void flattenTerrainForFeature(WorldGenRegion primer, int size, int x, int z, int dx, int dy, int dz) {
        ...
        for (int y = 0; y < mazeHeight; y++) {
            ...
            BlockPos old1 = primer.getCenter().getWorldPosition().offset(x, 0, z);
            if(!primer.getBiome(old1.atY(y)).is(ModBiomes.AIR)) {
                if (b.isAir() || b.liquid()) {
                    BlockPos old = primer.getCenter().getWorldPosition().offset(x, 0, z);
