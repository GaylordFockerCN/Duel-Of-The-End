...
public class WaveRiver {
    public static void main(String[] args) {
        int width = 100;
        int height = 100;
        int[][] river = generateIntersectingRivers(width, height, 8); // 生成5条交错的河流
    ...
    public static int[][] generateIntersectingRivers(int width, int height, int numRivers) {
        int[][] river = new int[height][width];
        ...
        for (int r = 0; r < numRivers; r++) {
            ...
            while (currentX >= startXOffset && currentX < width - startXOffset && currentY >= startYOffset && currentY < height - startYOffset) {
                double sinValue = Math.sin(2 * Math.PI * (currentX - startXOffset) / (innerWidth - 1))+Math.min(currentX-startXOffset,width-startXOffset-currentX); // 计算sin值，加上赋值使其中间不接地
                river[currentY][currentX] = (int)(Math.abs(sinValue) * 300) + rand.nextInt(1); // 将sin值的绝对值填入河流位置
